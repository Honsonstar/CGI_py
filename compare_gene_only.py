...
def run_gene_project():
    ...
    cmd = [
        ...
        "--results_dir", "case_results_compare_CFdemo_gene",
        "--batch_size", "1",
        "--lr", "0.0005",
        "--opt", "radam",
        "--reg", "0.0001",
        "--alpha_surv", "0.5",
        "--weighted_sample",
        "--max_epochs", "20",  # 修复: 从5改为20，与SNN.sh一致
    ...
    log_file = "logs/compare_CFdemo_gene.log"
    os.makedirs("logs", exist_ok=True)

    with open(log_file, 'w') as f:
        f.write(f"CFdemo_gene 仅基因模式测试\n")
...
def run_gene_text_project():
    ...
    cmd = [
        ...
        "--which_splits", "5foldcv_ramdom",
        "--omics_dir", "preprocessing/CPCG_algo/raw_data/finalstage_result_/tcga_brca/tcga_brca_M2M3base_0916.csv",
        "--results_dir", "case_results_compare_CFdemo_gene_text",
        "--batch_size", "1",
        "--lr", "0.0005",
        "--text_lr", "0.0001",
        "--gene_lr", "0.0003",
    ...
    log_file = "logs/compare_CFdemo_gene_text.log"
    os.makedirs("logs", exist_ok=True)

    with open(log_file, 'w') as f:
        f.write(f"CFdemo_gene_text 仅基因模式测试\n")
        f.write(f"启动时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("="*80 + "\n")
...
def wait_for_completion(process, name, timeout=1800):
    """等待进程完成"""
    print(f"\n⏳ 等待 {name} 完成...")
    try:
...
def main():
    ...
    result1 = parse_results("CFdemo_gene", "case_results_compare_CFdemo_gene")
    result2 = parse_results("CFdemo_gene_text", "case_results_compare_CFdemo_gene_text")
    ...
    if result1 and result2:
        ...
        print(f"   C-index: {cindex_diff:+.4f}")
        print(f"   IPCW:    {ipcw_diff:+.4f}")
        print(f"   IBS:     {ibs_diff:+.4f}")
        print(f"   IAUC:    {iauc_diff:+.4f}")
