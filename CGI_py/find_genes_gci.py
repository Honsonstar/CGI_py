...
def find_genes_gci(data: np.ndarray, alpha: float = 0.05,
    ...
    for i in range(n):
        ...
        # --- DEBUG BLOCK START ---
        if i < 10:
            print(f"DEBUG Gene {i}: PaCoTest (Linear Indep) = {ind1}")
        # --- DEBUG BLOCK END ---

        if ind1:
            ind2, stat, p_val_kcit = kcit(x, data[:, i], np.array([[]]), alpha=alpha)

            # --- DEBUG BLOCK START ---
            if i < 10:
                print(f"DEBUG Gene {i}: KCIT (Non-linear Indep) = {ind2}, p_val={p_val_kcit:.4f}")
            # --- DEBUG BLOCK END ---
