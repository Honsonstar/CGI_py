...
class SurvPath(nn.Module):
    def __init__(
        ...
        self.to_logits_gene = nn.Sequential(
                nn.Linear(self.wsi_projection_dim, int(self.wsi_projection_dim/4)),
                nn.ReLU(),
                nn.Linear(int(self.wsi_projection_dim/4), self.num_classes)
            )
        
        self.path_attention = torch.nn.MultiheadAttention(embed_dim=256, num_heads=4, batch_first=True)
    ...
    def forward(self, **kwargs):
        ...
        attn_output, attn_weights = self.path_attention(h_omic_bag, h_omic_bag, h_omic_bag)

        fused_feature =  torch.mean(attn_output, dim=1)  # 形状: [1, 256]
        
        #---> get logits
        logits = self.to_logits(fused_feature)
        print('logits shape',logits.shape) # torch.Size([1, 4]) (batch_size, num_classes’）
        if return_attn:
            return logits, attn_weights, None, None
        else:
