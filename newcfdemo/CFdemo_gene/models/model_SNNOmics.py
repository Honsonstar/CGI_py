...
class SNNOmics(nn.Module):
    def __init__(self, omic_input_dim: int, model_size_omic: str='small', n_classes: int=4, 
        ...
        # === [新增] 加上这行打印！===
        print(f"\n[DEBUG] SNNOmics 初始化状态: only_omic = {self.only_omic}")
        if self.only_omic:
             print("[DEBUG] -> 正在构建 [纯基因] 分类器 (Input Dim: 256)")
        else:
             print("[DEBUG] -> 正在构建 [多模态] 分类器 (Input Dim: 1024/1280)")
        # ==========================
    ...
    def forward(self, return_feats=False, **kwargs):
        x = kwargs['data_omics']
        gene_level_rep = self.fc_omic(x) # 1. 基因特征
        
        # === 初始化所有可能返回的变量 (防止 UnboundLocalError) ===
        logits_text = None
        logit_te = None
        logits_tie = None
        logits_nde = None
        ...
        if not self.only_omic:
            # ==============================
            # 模式 A: 基因 + 文本 (Gene + Text)
            # ==============================
            ...
            # 2. 通路与注意力机制
            use_pathway = getattr(self, 'use_pathway', True)
            if use_pathway:
            ...
            # 这里的变量已经在开头初始化为 None 或 {} 了，所以无需重复定义
            # text_pred = None
            # explanation_data = {} 

        # === 多任务分期预测 (Multitask Stage) ===
        if self.enable_multitask:
        ...
        # === 去偏逻辑 (De-bias Logic) ===
        # 只有在非 only_omic 模式下才能计算文本相关的去偏 Loss
        if self.use_debias and not self.only_omic:
             logit_te = self.fusion(survival_logits, text_pred, gene_pred, main_fact=True, text_fact=True, gene_fact=False)
             logits_nde = self.fusion(survival_logits.clone().detach(), text_pred.clone().detach(), gene_pred, main_fact=False, text_fact=True, gene_fact=False)
             if self.weight_debiase:
        ...
        # === 返回结果 ===
        # 根据 self.use_debias 和 self.enable_multitask 返回不同组合
        # 确保包含 explanation_data 以匹配你的需求
        ...
        # 这是一个常见的返回组合 (Survival + De-bias Logits + Explanation)
        if self.use_debias: 
