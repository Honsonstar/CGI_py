...
def _prepare_for_experiment(args):
    ...
    print(args.device)
    # ----> split dir 兼容逻辑
    # 1) 默认：splits/<which_splits>/<study>
    # 2) 如果用户传了 --split_dir:
    #    - 传的是 "splits"（你们脚本常用），则仍按默认拼接
    #    - 传的是一个具体目录，则直接使用该目录
    if getattr(args, "split_dir", None):
        if args.split_dir == "splits":
            args.split_dir = os.path.join("splits", args.which_splits, args.study)
        else:
            # 用户显式指定 splits 目录（完整路径/相对路径都可）
    ...
    # === 调试代码（已简化，仅在路径不存在时打印详细信息） ===
    if not os.path.isdir(args.split_dir):
        print(f"\n[DEBUG] Split dir not found!")
        print(f"[DEBUG] CWD: {os.getcwd()}")
        print(f"[DEBUG] split_dir: {args.split_dir}")
        print(f"[DEBUG] Full path: {os.path.abspath(args.split_dir)}")
    # === 调试代码结束 ===
    ...
    settings = {'num_splits': args.k, 
                ...
                'multitask_weight': args.multitask_weight if hasattr(args, 'multitask_weight') else 0.3
                }
...
def _get_custom_exp_code(args):
    ...
    # Add multitask parameters
    if args.enable_multitask:
        param_code += "_multitask_" + str(args.multitask_weight)
...
def _collate_omics(batch):
    ...
    Returns:
        ...
        - stage_labels : torch.LongTensor (if multitask enabled)
    ...
    # Check if multitask (stage labels present)
    if len(batch[0]) > 6:
        stage_labels = torch.LongTensor([item[6].long() for item in batch])
        return [img, omics, label, event_time, c, clinical_data_list, stage_labels]
    else:
...
def _collate_wsi_omics(batch):
    ...
    # Check if multitask (stage labels present)
    if len(batch[0]) > 7:
        stage_labels = torch.LongTensor([item[7].long() for item in batch])
        return [img, omics, label, event_time, c, clinical_data_list, mask, stage_labels]
    else:
...
def _collate_MCAT(batch):
    ...
    # Check if multitask (stage labels present)
    if len(batch[0]) > 12:
        stage_labels = torch.LongTensor([item[12].long() for item in batch])
        return [img, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, clinical_data_list, mask, stage_labels]
    else:
...
def _collate_survpath(batch):
    ...
    if len(batch[0]) > 7:
        stage_labels = torch.LongTensor([item[7].long() for item in batch])
        return [img, omic_data_list, label, event_time, c, clinical_data_list, mask, stage_labels]
    else:
