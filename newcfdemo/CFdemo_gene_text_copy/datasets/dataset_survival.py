...
class SurvivalDatasetFactory:
    ...
    def __init__(self,
        ...
        #---> load all clinical data first (needed for multitask filtering)
    ...
    def _load_clinical_data(self):
        ...
        """
        # Extract study name without 'tcga_' prefix for file path
        study_name = self.study.replace('tcga_', '') if self.study.startswith('tcga_') else self.study
        path_to_data = "./datasets_csv/clinical_data/tcga_{}_clinical.csv".format(study_name)
        
        if not os.path.exists(path_to_data):
            raise FileNotFoundError(f"Clinical data file not found: {path_to_data}")
        
        # Load clinical data - DON'T use index_col=0, case_id is a regular column
        self.clinical_data = pd.read_csv(path_to_data)
        
        # Set case_id as index if it exists as a column
        if 'case_id' in self.clinical_data.columns:
        ...
        if self.print_info:
            ...
            print(f"Clinical data index (case_id) samples: {self.clinical_data.index[:5].tolist()}")
            if 'stage' in self.clinical_data.columns:
                print(f"Available stages: {self.clinical_data['stage'].value_counts()}")
        
        # Process stage information for multitask learning
        if self.enable_multitask:
            if 'stage' not in self.clinical_data.columns:
                raise ValueError("Stage column not found in clinical data. Multi-task learning requires stage information.")
            self._process_stage_labels()
    
    def _process_stage_labels(self):
        r"""
        Process stage labels and create stage mapping for multitask learning
        ...
        Returns:
            - None
        """
        # Get valid stages (exclude '0' which typically means missing/invalid)
        valid_stages = self.clinical_data['stage'].dropna()
        valid_stages = valid_stages[valid_stages != '0']  # Exclude '0' stage
        unique_stages = valid_stages.unique()
        
        # Create stage mapping (sorted for consistency)
        self.stage_to_idx = {stage: idx for idx, stage in enumerate(sorted(unique_stages))}
        self.idx_to_stage = {idx: stage for stage, idx in self.stage_to_idx.items()}
        self.n_stage_classes = len(self.stage_to_idx)
        
        if self.print_info:
            print(f"Valid stage mapping (excluding '0'): {self.stage_to_idx}")
            print(f"Number of valid stage classes: {self.n_stage_classes}")
            
            # Show distribution of valid stages
            valid_stage_counts = valid_stages.value_counts()
            print(f"Valid stage distribution:")
            for stage, count in valid_stage_counts.items():
                print(f"  {stage}: {count}")
        
        if self.n_stage_classes == 0:
            raise ValueError("No valid stage information found for multitask learning!")
    ...
    def _setup_omics_data(self):
        ...
        for modality in ALL_MODALITIES:
            if self.omics_dir.endswith('.csv'):
                omics_path = self.omics_dir
            else:
                omics_path = os.path.join(self.omics_dir, 'rna_clean.csv')

            # 读取文件
            df_temp = pd.read_csv(omics_path, index_col=0)
            
            # === [新增] 剔除非特征列 (防止把 OS 当成基因) ===
            # 如果你的 CSV 里还有 'Censor' 或 'hazard_OD'，也要写在这里
            columns_to_drop = ['OS', 'Censor', 'hazard_OD'] 
            
            # 只删除存在的列，避免报错
            existing_drop_cols = [c for c in columns_to_drop if c in df_temp.columns]
            if existing_drop_cols:
                print(f"🧹 [Dataset] Dropping non-feature columns: {existing_drop_cols}")
                df_temp = df_temp.drop(columns=existing_drop_cols)
                
            self.all_modalities[modality.split('_')[0]] = df_temp
            # ==================================================
            #修改0117
    ...
    def _clean_label_data(self):
        ...
        If multitask is enabled, filter out samples without stage information.
        ...
        if self.enable_multitask:
            initial_count = len(self.label_data)

            # Ensure case_id data types match for merging
            self.label_data['case_id'] = self.label_data['case_id'].astype(str)
            self.clinical_data.index = self.clinical_data.index.astype(str)
            
            # Check for overlap
            label_case_ids = set(self.label_data['case_id'].unique())
            clinical_case_ids = set(self.clinical_data.index.unique())
            overlap = label_case_ids.intersection(clinical_case_ids)

            # If direct overlap exists, proceed with merging
            if len(overlap) > 0:
                if self.print_info:
                    print("Using direct case_id matching for merging")
            # Try truncating label case_ids to match clinical format (usually first 12 characters for TCGA)
            elif len(label_case_ids) > 0 and len(clinical_case_ids) > 0:
                # Try truncating to 12 characters (standard TCGA patient ID length)
                self.label_data['case_id_truncated'] = self.label_data['case_id'].str[:12]
                label_case_ids_truncated = set(self.label_data['case_id_truncated'].unique())
                overlap_truncated = label_case_ids_truncated.intersection(clinical_case_ids)
                
                if self.print_info:
                    print(f"After truncation - overlapping case_ids: {len(overlap_truncated)}")
                
                if len(overlap_truncated) > 0:
                    # Use truncated case_id for merging
                    self.label_data['case_id'] = self.label_data['case_id_truncated']
                    if self.print_info:
                        print("Using truncated case_id for merging")
                else:
                    print("ERROR: Still no overlap after truncation!")
                    # If multitask is enabled but no overlap found, raise error
                    raise ValueError("No matching case_ids found between label data and clinical data. Cannot perform multitask learning without stage information.")
            else:
                print("ERROR: No case_ids available for comparison!")
                raise ValueError("No case_ids available for comparison between label data and clinical data.")
            
            # Merge with clinical data to check stage availability
            # [Fix] 检查 stage 是否存在
            if 'stage' in self.label_data.columns:
                merged_data = self.label_data
            else:
                merged_data = self.label_data.merge(
                    self.clinical_data[['stage']], 
                    left_on='case_id', 
                    right_index=True, 
                    how='left'
                )
            ...
            # Keep only samples with valid stage information (not NaN and not '0')
            valid_stage_mask = merged_data['stage'].notna() & (merged_data['stage'] != '0')
            self.label_data = merged_data[valid_stage_mask].copy()
            final_count = len(self.label_data)

        self.patients_df = self.label_data.drop_duplicates(['case_id']).copy()

        # [DEBUG] 仅在找不到列时打印调试信息
        if self.censorship_var not in self.patients_df.columns:
            print(f'[DEBUG] Column "{self.censorship_var}" not found!')
            print(f'[DEBUG] Available columns: {list(self.patients_df.columns)}')
    ...
    def _summarize(self):
        ...
        if self.print_info:
            ...
            if self.enable_multitask:
                ...
                print("number of stage classes: {}".format(getattr(self, 'n_stage_classes', 'Not set')))
    ...
    def return_splits(self, args, csv_path, fold):
        ...
        if 'val' in all_splits.columns:
            val_key = 'val'
        elif 'test' in all_splits.columns:
            val_key = 'test'
        else:
            print(f"❌ 切分文件列名: {all_splits.columns}")
    ...
    def _get_split_from_df(self, args, all_splits, split_key: str='train', fold = None, scaler=None, valid_cols=None, custom_omics_dict=None):
        ...
        for key in modalities_source.keys():
            ...
            clinical_data_mask = self.clinical_data.index.isin(split.tolist())
        ...
        split_dataset = SurvivalDataset(
            ...
            stage_to_idx=getattr(self, 'stage_to_idx', None)
            )
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.modality in ['omics', 'snn', 'mlp_per_path']:
            ...
            if self.enable_multitask:
                stage_label = self.get_stage_label(case_id)
                return (torch.zeros((1,1)), omics_tensor, label, event_time, c, clinical_data, stage_label)
            else:
        ...
        #@TODO what is the difference between tmil_abmil and transmil_wsi
            ...
            if self.enable_multitask:
                stage_label = self.get_stage_label(case_id)
                return (patch_features, omics_tensor, label, event_time, c, clinical_data, mask, stage_label)
            else:
            ...
            if self.enable_multitask:
                stage_label = self.get_stage_label(case_id)
                return (patch_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, clinical_data, mask, stage_label)
            else:
            ...
            if self.enable_multitask:
                stage_label = self.get_stage_label(case_id)
                return (patch_features, omic_list, label, event_time, c, clinical_data, mask, stage_label)
            else:
    ...
    def get_stage_label(self, case_id):
        r"""
        Get stage label for multitask learning
        ...
        """
        if not self.enable_multitask or self.stage_to_idx is None:
            return torch.tensor(-1)  # Return invalid label if multitask is disabled
        
        try:
            # Ensure case_id is string for consistent lookup
            case_id = str(case_id)
            stage = self.clinical_data.loc[case_id, "stage"]
            
            # Handle invalid stages ('0', NaN, 'N/A')
            if pd.isna(stage) or stage == "N/A" or stage == '0':
                return torch.tensor(-1)  # Invalid stage
                
            stage_idx = self.stage_to_idx.get(str(stage), -1)
            return torch.tensor(stage_idx)
        except (KeyError, IndexError):
            return torch.tensor(-1)  # Default to invalid if error occurs
    ...
    def get_clinical_data(self, case_id):
        ...
        try:
            text_report = self.clinical_data.loc[case_id, "text_report"]
        except:
            text_report = "N/A"
        # text_report = self.clinical_data.loc[case_id, "text_report"]
        clinical_data = (stage, grade, subtype, text_report)
