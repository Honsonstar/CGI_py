...
class SNNOmics(nn.Module):
    def __init__(self, study_name, omic_input_dim: int, model_size_omic: str='small', n_classes: int=4, 
        ...
        self.use_pathway = True
         #这里需要修改到指向新的基因通路文件
        # clean_study = study_name.replace('tcga_', '')
        # pathway_file = f'./datasets_csv/pathway_compositions/custom_{clean_study}_comps.csv'
        #改用原来的通路文件
        pathway_file = './datasets_csv/pathway_compositions/combine_comps.csv'
        print(f"[*] Loading Pathway File: {pathway_file}")
        ...
        if self.multi_model:    
            ...
            if self.use_pathway:
                ...
                self.survival_classifier = nn.Linear(classifier_input_dim, n_classes)
            else:
